...
clients = set()
last_v, last_y, last_x = 0, 50, 50   # 마지막 상태를 모두 기억
...
async def handler(ws, path):
    global last_v, last_y, last_x
    clients.add(ws)
    try:
        # 새로 연결된 클라이언트에 마지막 상태 1회 전송
        await ws.send(make_packet(last_v, last_y, last_x))
        # 이후 수신되는 모든 메시지를 전체 브로드캐스트
        async for msg in ws:
            # (v,y,x) 형식만 온다고 가정. 필요시 validate 추가
            m = re.fullmatch(r"\((\d+),(\d+),(\d+)\)", msg.strip())
            if m:
                last_v, last_y, last_x = map(int, m.groups())
...
def parse_line(line: str):
    ...
    if m:
        v, y, x = map(int, m.groups())
        last_v, last_y, last_x = v, y, x   # ← 마지막 상태 갱신
        return v, y, x
